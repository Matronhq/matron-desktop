...
_asar_cfg="$(mktemp -d)/config.json"
if npx asar extract-file webapp.asar config.json > "$_asar_cfg" 2>/dev/null; then
  if grep -qE "default_server|matrix-client\.matrix\.org|scalar|vector\.im|gitter" "$_asar_cfg"; then
